...
public final class Buffer implements Cloneable, BufferedSink, BufferedSource {
    ...
    public Buffer copyTo(Buffer out, long offset, long byteCount) {
        ...
        if (byteCount != 0) {
            ...
            out.size += byteCount;
            while (byteCount > 0) {
                while (offset >= ((long) (source.limit - source.pos))) {
                    offset -= (long) (source.limit - source.pos);
                    source = source.next;
                }
                if (target.limit == 2048) {
                    target = target.push(SegmentPool.INSTANCE.take());
                }
                int toCopy = (int) Math.min(Math.min(((long) source.limit) - (((long) source.pos) + offset), byteCount), (long) (2048 - target.limit));
        ...
        while (i < length) {
            ...
            if (c < 128) {
                ...
                if (c > 56319 || low < 56320 || low > 57343) {
                    ...
                    int codePoint = NativeProtocol.MESSAGE_GET_ACCESS_TOKEN_REQUEST + (((-55297 & c) << 10) | (-56321 & low));
