...
class k {
    ...
    static i a(s sVar) {
        try {
            ...
            if (b == '\"' || b == '\'') {
                e = e(sVar);
                if (e.length() == 0) {
                    return new f(BuildConfig.VERSION_NAME);
                }
                b = e.charAt(0);
                if (b == 't' && e.equals(ServerProtocol.DIALOG_RETURN_SCOPES_TRUE)) {
                    return ADCData.a;
                }
    ...
    static int a(int i) {
        ...
        if (i >= 97 && i <= FacebookRequestErrorClassification.EC_INVALID_SESSION) {
            return (i - 97) + 10;
        }
        if (i < 65 || i > 70) {
            return 0;
